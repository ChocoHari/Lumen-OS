...
export default function CourseWorkspacePage() {
  ...
  useEffect(() => {
    async function loadWorkspace() {
      const [workspaceCourse, workspaceNotes, workspaceRewrites, workspaceAssignments, allTasks] = await Promise.all([
        db.courses.get(courseId),
        db.notes.where('courseId').equals(courseId).toArray(),
        db.rewrites.where('courseId').equals(courseId).toArray(),
        db.assignments.where('courseId').equals(courseId).toArray(),
        db.tasks.toArray()
      ]);
      const assignmentIds = new Set(workspaceAssignments.map((assignment) => assignment.id).filter(Boolean) as number[]);
      setCourse(workspaceCourse ?? null);
      setNotes(workspaceNotes);
      setRewrites(workspaceRewrites);
      setAssignments(workspaceAssignments);
      setTasks(allTasks.filter((task) => assignmentIds.has(task.assignmentId)));
    }
        ...
        <div className="mx-auto grid max-w-7xl gap-5 lg:grid-cols-[260px_1fr]">
          ...
          <section>
            ...
            <div className="rounded-xl2 border border-gray-100 bg-white p-4 text-sm">
              ...
              {activeTab === 'Rewrite' && rewrites.map((rewrite) => <p key={rewrite.id}>Note #{rewrite.noteId}: {rewrite.wordCount} words · {rewrite.status}</p>)}
